# fault2graph Phase B — Power-Annex COMMON PROTOCOL v1 (shared by annexes B-1/B-2/B-3)

**Author:** cayley — **Date:** 2026-08-20 — **Status:** DRAFT until the three family annexes are hash-bound in the seal manifest. This file is referenced BY DIGEST from each family annex; it is not itself a manifest slot.
...
- **Panel geometry (real-carrier scale):** 3 carriers with 12/12/11 stations (35 total, matching the frozen selected registry count), 110 registered days each (positions 1–60 baseline / 61–110 evaluation), segment labels alternating two segments per carrier by sorted station index. **Missingness:** 8% MCAR at the (edge, day) cell level (matching the Phase-A order of magnitude, 272/3849 ≈ 7%), applied before any effect injection.
...
- **Tier-C `CERTIFICATION` (exact production pipeline):** `n_draws = 9,999`, frozen valid-draw floor, add-one p, and the final **post-LOCO family-positive rule**: a replicate counts as a recovery only if the full-data family passes α = 0.05/3 AND `loco_gate` passes over ALL 35 station-drop folds (each fold its own frozen `loco:<STATION_ID>` null substream). Pre-LOCO and post-LOCO recovery are reported separately; **the MDE claim follows post-LOCO recovery.**
- **Tier-C candidate selection (registered, deterministic):** the 3 grid points with highest Tier-S post-α recovery per family; ties broken lexicographically by grid coordinates as listed in the family annex.
...
- **No-power contract route:** a family registers `MDE_NOT_ATTAINED` only via (a) a mathematical support proof encoded and executably checked (B-2 annex §L), and/or (b) Tier-C evidence that every selected candidate FAILED, under this registered upper-bound rule: Tier-C candidates are the Tier-S argmax cells, so their failure bounds the surveyed grid; cells outside the grid are not claimed, and the family's nonpositive typing per frozen §5 (`CANNOT_DETERMINE_NO_POWER`) applies to everything not certified.
