...
- **station query (RECEIPT + ENVELOPE, codex freeze-review fix 3)**: FDSN station service,
  networks **UW, CC, CN**, channel **HHZ**, level=channel, window 2026-07-11→2026-11-30.
  Body: `receipts/cascadia_UW_CC_CN_HHZ.txt`, sha256
  `d4256792bf85edf855a4dbaf7841982824a020cd5e075c103d832248c513a847` (byte-identical across the
  original and envelope refires). **Authority = the JSON receipt ENVELOPE**
  `receipts/cascadia_UW_CC_CN_HHZ.envelope.json`: requested URL + query, effective URL after
  redirects (`service.earthscope.org/...`), fired UTC, HTTP status, selected response headers,
  body path + sha256, parser version, and recomputed counts (203 channel rows, **198 unique
  net.sta**: UW 118, CC 43, CN 37) — every claimed fact recomputable from pinned bytes.
- **duplicate-identity + epoch resolution (codex freeze-review fix 3)**: identity = `NET.STA`
  (network is part of identity). Channel epochs are half-open `[start, end)`; **for each
  required time, FIRST restrict to epochs ACTIVE at that time, THEN apply location precedence
  `blank → 00 → lowest lexicographic`** — a literally-blank-first rule would select dead epochs
  (the pinned body's `UW.TOUT..HHZ` ends 2026-07-16 exactly as `UW.TOUT.00.HHZ` opens). The
  per-day epoch/location schedule is preserved while identity collapses to `NET.STA`.
  **KATs (W-CAS)**: the TOUT transition; RER's three adjacent epochs; simultaneously active
  blank/00 rows; envelope-vs-body recomputation.
