...
Registered objects (codex 1400Z, verbatim contract):
  ...
  excluded_days   = [2026-08-26]              (PRESTART; never an
                                               engine position)
...
Cutoff binding (codex ratification): cutoff = 2026-08-25 is the
...
PRESTART day is excluded. If PRESTART does not complete on 08-26 the
...
def build():
    baseline = _span("2026-06-27", "2026-08-25")
    evaluation = _span("2026-08-27", "2027-01-05")
    engine = baseline + evaluation
    assert len(baseline) == 60 and len(evaluation) == 132
    assert len(engine) == 192 and "2026-08-26" not in engine
    ...
    assert (len(engine) - b1b["baseline_positions"]) \
        // b1b["block_len"] == 11
    frame = {"frame_id": "w2-calendar-v2-noncal",
             ...
             "b1b": b1b}
    return {
        "schema": "f2g-w2-calendar-authority-v2",
        "frame": frame,
        "digests": {
        ...
        "cutoff_binding": {
            ...
            "rule": "the greatest UTC date whose complete-day bytes "
                    ...
                    "PRESTART day is excluded; a missed 08-26 "
        ...
        "provenance": {
            "ruling": "codex 2026-08-23T1400Z ruling 1 (calendar "
                      ...
                      "contract)",
            "owner_decision": "asylum in-session 'prestart 8/26 it "
                              "is' (quote sha 5b44934a, schedule "
                              "artifact b49ab2c)",
