#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Window-2 INDEPENDENTLY-EXPECTED RECORD CONTRACTS v4 generator
(cayley) -- APPEND-ONLY SUCCESSOR to the v3 generator under codex's
w2r1 cycle-2 ruling (2026-08-30T22:40Z, finding 1): the successor
calendar propagation stopped before the registered selection
authority. The selection cutoff/lookback here are DERIVED from the
committed calendar authority v4 -- never a second date literal --
and a v3/carry-forward authority REFUSES typed. The v3 generator
and staged_expected_contracts_v3.json stand as history.

Static layer of the producer_boundary BIND condition
...
# PER-LANE cutoffs: selection DERIVES from calendar v4 (below); the MAG
# lanes keep their historical 07-31 cap (captured, calibrated, bound)
...
V3_SUPERSEDED_SELECTION_CUTOFF = "2026-08-27"
CALENDAR_AUTHORITY_REL = os.path.join(
    "docs", "f2g_window2_execution",
    "calendar_authority_w2_v4.json")


def _derive_selection_frame(repo):
    """codex cycle-2 finding 1: derive (cutoff,
    lookback_start) from the COMMITTED calendar authority v4.
    Fail-closed: schema/frame_id/count/digest/consecutive
    mismatches refuse typed, and a cutoff that does not move
    strictly past the superseded v3 selection cutoff refuses
    as v3 carry-forward (dates alone can never pass)."""
    def refuse(detail):
        raise AssertionError(
            "SELECTION_AUTHORITY_REFUSED: " + detail)
    with open(os.path.join(repo, CALENDAR_AUTHORITY_REL),
              encoding="utf-8") as f:
        auth = json.load(f)
    if auth.get("schema") != "f2g-w2-calendar-authority-v4":
        refuse(f"calendar schema {auth.get('schema')!r} is not v4")
    frame = auth.get("frame") or {}
    if frame.get("frame_id") != "w2-calendar-v4-noncal":
        refuse(f"frame_id {frame.get('frame_id')!r} is not v4")
    base = list(frame.get("baseline_days") or [])
    ev = list(frame.get("evaluation_days") or [])
    eng = list(frame.get("engine_days") or [])
    exc = list(frame.get("excluded_days") or [])
    if len(base) != 60 or len(ev) != 132 or len(eng) != 192:
        refuse("frame counts are not 60/132/192")
    if eng != base + ev or eng != sorted(eng) or \
            len(set(eng)) != 192:
        refuse("engine days are not canonical baseline||evaluation")
    if len(exc) != 1 or exc[0] in eng:
        refuse("excluded day set is not one off-grid PRESTART day")

    def _dig(days):
        return hashlib.sha256(json.dumps(
            list(days), separators=(",", ":")).encode()
            ).hexdigest()
    d = auth.get("digests") or {}
    if (d.get("baseline_days_sha256") != _dig(base)
            or d.get("evaluation_days_sha256") != _dig(ev)
            or d.get("engine_days_sha256") != _dig(eng)):
        refuse("registered digests do not recompute from the frame")
    cutoff = (auth.get("cutoff_binding") or {}).get("cutoff")
    if cutoff != base[-1]:
        refuse("cutoff_binding diverges from baseline end")
    ds = datetime.date.fromisoformat
    if ds(cutoff) + datetime.timedelta(days=1) != \
            ds(exc[0]) or \
            ds(exc[0]) + datetime.timedelta(days=1) != \
            ds(ev[0]):
        refuse("cutoff/PRESTART/evaluation_start are not consecutive")
    if cutoff <= V3_SUPERSEDED_SELECTION_CUTOFF:
        refuse(f"derived cutoff {cutoff} does not move strictly past "
               f"the superseded selection cutoff "
               f"{V3_SUPERSEDED_SELECTION_CUTOFF} (v3 carry-forward)")
    lookback = (ds(cutoff)
                - datetime.timedelta(days=89)).isoformat()
    return cutoff, lookback


_REPO_OF_MODULE = os.path.abspath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", ".."))
SELECTION_CUTOFF, SELECTION_LOOKBACK_START = \
    _derive_selection_frame(_REPO_OF_MODULE)
...
def build(repo):
    ...
    return {
        "schema": "f2g-w2-expected-contracts-v4",
        ...
        "provenance": {
            "producer": "monitoring/src/"
                        "w2_expected_contracts_gen_v4_cayley.py",
...
def _selftest():
    ...
    assert art["schema"] == "f2g-w2-expected-contracts-v4"
    ...
    # --- cycle-3 negative doctors (codex cycle-2 finding 1
    # item 4): an 08-27/v3 authority cannot satisfy the
    # successor gate. Each doctor CONSTRUCTS its precondition
    # from the real committed artifacts; the unmutated
    # control passes first; a relabeled v3 frame (v4 strings,
    # v3 dates, honestly recomputed digests) refuses on the
    # carry-forward bound -- dates alone can never pass.
    assert SELECTION_CUTOFF == "2026-09-02" and \
        SELECTION_LOOKBACK_START == "2026-06-05"
    sel = art["prestart_expected_keys"]["SELECTION_RECORDS"]
    for _ck, _days in sel.items():
        assert len(_days) == 90 and \
            _days[0] == SELECTION_LOOKBACK_START and \
            _days[-1] == SELECTION_CUTOFF, _ck
    import copy as _copy
    import shutil as _shutil
    import tempfile as _tempfile
    with _tempfile.TemporaryDirectory() as _td:
        os.makedirs(os.path.join(
            _td, "docs", "f2g_window2_execution"))
        _dst = os.path.join(_td, CALENDAR_AUTHORITY_REL)
        _shutil.copyfile(os.path.join(
            repo, CALENDAR_AUTHORITY_REL), _dst)
        assert _derive_selection_frame(_td) == (
            SELECTION_CUTOFF, SELECTION_LOOKBACK_START)
        ...
        def _expect(doc, why):
            with open(_dst, "w", encoding="utf-8") as _f:
                json.dump(doc, _f)
            try:
                _derive_selection_frame(_td)
                raise SystemExit(
                    "v4 selection doctor must refuse: " + why)
            except AssertionError as _ex:
                assert "SELECTION_AUTHORITY_REFUSED" in \
                    str(_ex) and why in str(_ex), \
                    (why, str(_ex))
        _expect(_cal3, "is not v4")
        _rel = _copy.deepcopy(_cal3)
        _rel["schema"] = "f2g-w2-calendar-authority-v4"
        _rel["frame"]["frame_id"] = "w2-calendar-v4-noncal"
        _expect(_rel, "v3 carry-forward")
        _tam = _copy.deepcopy(_cal4)
        _d0 = _tam["digests"]["engine_days_sha256"]
        _tam["digests"]["engine_days_sha256"] = (
            ("0" if _d0[0] != "0" else "1") + _d0[1:])
        _expect(_tam, "do not recompute")
        _div = _copy.deepcopy(_cal4)
        _div["cutoff_binding"]["cutoff"] = \
            _div["frame"]["evaluation_days"][0]
        _expect(_div, "diverges from baseline end")
    print("w2_expected_contracts_gen_v4 selftest: ALL PASS (all-ten "
          "committed evidence locks executed via build(); mutation "
          "doctors refuse through the REAL validator; "
          "canonical-builder comparison; v4 selection-authority "
          "doctors: v3/relabeled/digest/cutoff all refuse)")
